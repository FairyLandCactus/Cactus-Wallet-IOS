...
-(void)dealloc{
    ...
    notification_remove(self, @"KRefreshWallet", nil);
}
...
- (void)viewDidLoad {
    ...
    [self createSubViews];
    self.solPrice = [DTUserDefaults getDoubleForKey:@"KSOL_USDTPrice"];
    self.BNBBalance = [DTUserDefaults getStringForKey:@"KBNBBalance"];
    self.ETHBalance = [DTUserDefaults getStringForKey:@"KETHBalance"];
    self.wSol = @"0.00";
    if (kStringIsEmpty(self.BNBBalance)) {
        self.BNBBalance = @"0.00";
    }
    if (kStringIsEmpty(self.ETHBalance)) {
...
//    NSString *url = @"https://walletadmin.ius.finance/index.php/api/cover/index?language=zh_CN";
...
-(void)setSelectWalletAddress{
    ...
    if (self.WalletChain == 1) {
        array = [DTUserDefaults getArrayForKey:@"SOLWalletList"];
    }else if (self.WalletChain == 2){
        array = [DTUserDefaults getArrayForKey:@"BNBWalletList"];
    }else if (self.WalletChain == 3){
        array = [DTUserDefaults getArrayForKey:@"ETHWalletList"];
    }else if (self.WalletChain == 4){
        array = [DTUserDefaults getArrayForKey:@"BTCWalletList"];
    }
            ...
            if ([obj[@"isSelect"] intValue]) {
                ...
                if ([obj[@"name"] containsString:@"BNB"]) {//币安
                    ...
                    dispatch_async(queue, ^{
                        ...
                        //查BNB余额
                        [self loadBNB_Balance:obj[@"address"] Chain:@"BNB"];
                ...
                }else if ([obj[@"name"] containsString:@"ETH"]) {//以太坊
                    
                    OCBridgeSwift *pair = [OCBridgeSwift new];
                    self.pair = pair;
                    [self.list removeAllObjects];
                    [self.list addObject:@{@"tokenName":@"ETH",@"tokenIcon":@"Ethereum",@"Balance":self.ETHBalance}];
                    [self.tableView reloadData];
                    dispatch_queue_t queue = dispatch_queue_create("check_Balance", DISPATCH_QUEUE_CONCURRENT);
                    dispatch_async(queue, ^{
                        
                        //查ETH余额
                        [self loadBNB_Balance:obj[@"address"] Chain:@"ETH"];
                        
                        [self.list removeAllObjects];
                        [self.list addObjectsFromArray:self.currentWallet[@"tokenList"]];
                        for (NSDictionary*wallet in self.list) {
                            if (![wallet[@"name"] isEqualToString:@"ETH"]) {
                                //查代币余额
                               // [self loadBEP20_Token_Balance:obj[@"address"] contract:wallet[@"contractAdd"]];
                            }
...
-(void)checkTokenBalance:(NSNotification *)not{
  ...
  //  NSLog(@"%@",dic[@"contractAdd"]);
    ...
    NSMutableArray *array = [NSMutableArray arrayWithArray:[DTUserDefaults getArrayForKey:@"BNBWalletList"]];
    ...
    [DTUserDefaults setArray:array key:@"BNBWalletList"];
...
- (IBAction)manageWalletAction:(UIButton *)sender {
    ManageWalletView *walletView = [[ManageWalletView alloc] initManageWalletView:1];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if ([self.currentWallet[@"coin"] containsString:@"BNB"]) {
        ...
        if (indexPath.row == 0) {
            cell.coinImg.image = kImage(@"Ethereum");
            cell.coin.text = @"ETH";
            cell.coinNum.text = self.ETHBalance;
            cell.usMoney.text  = @"0.000USDT";
        }else{
...
//                }
                ...
                NSMutableArray *walletList = [NSMutableArray arrayWithArray:[DTUserDefaults getArrayForKey:@"SOLWalletList"]];
                ...
                [DTUserDefaults setArray:array key:@"SOLWalletList"];
...
#pragma mark=====获取BNB/ETH余额
-(void)loadBNB_Balance:(NSString*)address Chain:(NSString*)chain{
    
   // [self.pair checkBNBBalanceWithAddress:address];
    dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
        NSString *url ;
        if ([chain isEqualToString:@"BNB"]) {
            url = [NSString  stringWithFormat:@"https://api.bscscan.com/api?module=account&action=balance&address=%@&apikey=78NFN5UHK9FTYN6ZTSHZCK8IF5DV4W3JI5",self.currentWallet[@"address"]];
        }else if ([chain isEqualToString:@"ETH"]){
            url = [NSString  stringWithFormat:@"https://api.etherscan.io/api?module=account&action=balance&address=%@&tag=latest&apikey=FJV1AWZ8UWNMX9NIF3UN26NCDU1D5Y2SYG",self.currentWallet[@"address"]];
        }
    ...
    if ([result[@"status"] intValue]) {
        if ([chain isEqualToString:@"BNB"]) {
            ...
            self.ETHBalance = [NSString stringWithFormat:@"%f",[result[@"result"] integerValue]/1000000000000000000.0];
            [DTUserDefaults setString:self.ETHBalance key:@"KETHBalance"];
        }
